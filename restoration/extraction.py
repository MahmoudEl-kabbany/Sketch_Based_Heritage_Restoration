"""Phase 1 — Extraction.

Skeleton-based Bezier path extraction with endpoint tangents, curvatures,
and EFD coefficient extraction.
"""

from __future__ import annotations

import sys
import os
from dataclasses import dataclass, field
from typing import Dict, List, Optional, Tuple

import cv2
import numpy as np

# ---------------------------------------------------------------------------
# Project imports
# ---------------------------------------------------------------------------
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

from bezier_curves.bezier import (
    BezierPath,
    BezierSegment,
    fit_from_image_skeleton,
)
from eliptic_fourier_descriptors.efd import (
    reconstruct_contour_efd,
    compute_efd_features,
)


# ═══════════════════════════════════════════════════════════════════════════
# Endpoint Tangent Tuning Constants
# ═══════════════════════════════════════════════════════════════════════════

# Multiscale context estimation: window fractions and bounds for PCA analysis
ENDPOINT_WINDOW_FRACTIONS = [0.10, 0.15, 0.20, 0.25]
ENDPOINT_WINDOW_MIN = 8
ENDPOINT_WINDOW_MAX = 28
ENDPOINT_PCA_CONFIDENCE_LINEARITY_WEIGHT = 0.5
ENDPOINT_PCA_CONFIDENCE_DIRECTION_WEIGHT = 0.5

# Sampled polyline inspection for context tangent
ENDPOINT_SAMPLE_PTS_PER_SEGMENT = 24
ENDPOINT_SAMPLE_MIN_POINTS = 3
ENDPOINT_LOOKAHEAD_DISTANCE = 2  # steps ahead from start for direction
ENDPOINT_LOOKBACK_DISTANCE = 3   # steps back from end for direction

# Tangent blending between derivative and context estimates
ENDPOINT_CONFIDENCE_LOW_THRESHOLD = 0.40
ENDPOINT_BLEND_LOW_CONFIDENCE_MIN = 0.05
ENDPOINT_BLEND_LOW_CONFIDENCE_MAX = 0.35
ENDPOINT_BLEND_HIGH_CONFIDENCE_MIN = 0.40
ENDPOINT_BLEND_HIGH_CONFIDENCE_MAX = 0.88
ENDPOINT_INTERIOR_ALIGNMENT_THRESHOLD = 0.50
ENDPOINT_MISALIGNMENT_CONFIDENCE_PENALTY = 0.5


# ═══════════════════════════════════════════════════════════════════════════
# Data classes
# ═══════════════════════════════════════════════════════════════════════════

@dataclass
class EndpointInfo:
    """Geometric descriptor for one end of a BezierPath."""

    endpoint_id: int        # stable endpoint identity within one extraction
    path_index: int          # index into ExtractionResult.paths
    end: str                 # "start" | "end"
    position: np.ndarray     # (x, y)
    tangent: np.ndarray      # unit tangent pointing *outward* from the path
    curvature: float         # unsigned curvature κ at this endpoint
    tangent_confidence: float = 1.0  # confidence in endpoint tangent direction


@dataclass
class ExtractionResult:
    """Everything extracted from one image."""

    paths: List[BezierPath]
    endpoints: List[EndpointInfo]
    efd_contours: List[dict]       # per-contour EFD data
    image_shape: Tuple[int, int]   # (h, w)
    diagonal: float                # sqrt(h² + w²)
    dist_map: Optional[np.ndarray] = None  # distance map from lines


# ═══════════════════════════════════════════════════════════════════════════
# Tangent / curvature from Bezier control points
# ═══════════════════════════════════════════════════════════════════════════

def _bezier_derivative_at(cp: np.ndarray, t: float) -> np.ndarray:
    """First derivative of cubic Bezier at parameter *t*.

    cp: (4, 2) control points.
    """
    u = 1.0 - t
    return (
        3.0 * u * u * (cp[1] - cp[0])
        + 6.0 * u * t * (cp[2] - cp[1])
        + 3.0 * t * t * (cp[3] - cp[2])
    )


def _bezier_second_derivative_at(cp: np.ndarray, t: float) -> np.ndarray:
    """Second derivative of cubic Bezier at parameter *t*."""
    u = 1.0 - t
    return (
        6.0 * u * (cp[2] - 2.0 * cp[1] + cp[0])
        + 6.0 * t * (cp[3] - 2.0 * cp[2] + cp[1])
    )


def _curvature_at(cp: np.ndarray, t: float) -> float:
    """Unsigned curvature κ = |x'y'' - y'x''| / (x'² + y'²)^(3/2).

    Improvement #12: more conservative numerical stability guard and
    clamping to a physically reasonable maximum (κ > 1 means radius < 1 pixel).
    """
    d1 = _bezier_derivative_at(cp, t)
    d2 = _bezier_second_derivative_at(cp, t)
    cross = float(d1[0] * d2[1] - d1[1] * d2[0])
    speed_sq = float(d1[0] ** 2 + d1[1] ** 2)
    if speed_sq < 1e-8:  # More conservative than 1e-24
        return 0.0
    kappa = abs(cross) / (speed_sq ** 1.5)
    return min(kappa, 1.0)  # Clamp to physically reasonable maximum


def _safe_normalize(v: np.ndarray) -> np.ndarray:
    n = np.linalg.norm(v)
    if n < 1e-12:
        return np.array([1.0, 0.0], dtype=np.float64)
    return v / n


def _dedupe_polyline(points: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    """Remove near-duplicate consecutive points from a sampled polyline."""
    if len(points) <= 1:
        return points
    diffs = np.linalg.norm(np.diff(points, axis=0), axis=1)
    keep = np.ones(len(points), dtype=bool)
    keep[1:] = diffs > eps
    return points[keep]


def _strip_sway_prefix(points: np.ndarray, end: str, max_strip_fraction: float = 0.08) -> np.ndarray:
    """Walk inward from the endpoint and drop high-curvature sway segments.

    Improvement #10: conservative 40° threshold (was 25°) and 8% max strip
    (was 15%) to avoid removing real stroke data while still filtering out
    noisy junction artifacts.
    """
    if len(points) < 10:
        return points

    n_strip_max = int(round(len(points) * max_strip_fraction))
    if n_strip_max < 2:
        return points

    diffs = np.diff(points, axis=0)
    strip_count = 0
    sway_angle_threshold = 40.0  # Was 25.0 — more conservative
    
    if end == "start":
        for i in range(n_strip_max):
            a = diffs[i]
            b = diffs[i + 1]
            na = np.linalg.norm(a)
            nb = np.linalg.norm(b)
            if na < 1e-12 or nb < 1e-12:
                continue
            dot = float(np.clip(np.dot(a / na, b / nb), -1.0, 1.0))
            angle = float(np.degrees(np.arccos(dot)))
            if angle > sway_angle_threshold:
                strip_count = i + 1
            elif i > strip_count + 1:
                break
        if strip_count > 0:
            return points[strip_count:]
    else:
        for i in range(n_strip_max):
            idx = len(diffs) - 1 - i
            a = diffs[idx - 1]
            b = diffs[idx]
            na = np.linalg.norm(a)
            nb = np.linalg.norm(b)
            if na < 1e-12 or nb < 1e-12:
                continue
            dot = float(np.clip(np.dot(a / na, b / nb), -1.0, 1.0))
            angle = float(np.degrees(np.arccos(dot)))
            if angle > sway_angle_threshold:
                strip_count = i + 1
            elif i > strip_count + 1:
                break
        if strip_count > 0:
            return points[:-strip_count]

    return points


def _multiscale_context_tangent(
    sampled: np.ndarray, end: str, outward_ref: np.ndarray
) -> Tuple[np.ndarray, float]:
    """Run PCA at 4 different window sizes and return circular mean of directions.

    Improvement #11: uses exponential distance weighting in PCA so points
    nearer the endpoint contribute more to the tangent estimate.
    """
    directions = []
    weights = []
    n_pts = len(sampled)
    
    for frac in ENDPOINT_WINDOW_FRACTIONS:
        window = int(round(n_pts * frac))
        window = max(ENDPOINT_WINDOW_MIN, min(ENDPOINT_WINDOW_MAX, max(2, n_pts - 1), window))
        
        if end == "start":
            local = sampled[: window + 1]
        else:
            local = sampled[-(window + 1):]

        centered = local - np.mean(local, axis=0)

        # Distance-based weighting: points nearer the endpoint get higher weight.
        n_local = len(local)
        if end == "start":
            dists = np.arange(n_local, dtype=np.float64)
        else:
            dists = np.arange(n_local - 1, -1, -1, dtype=np.float64)
        dist_weights = np.exp(-0.15 * dists)
        dist_weights /= dist_weights.sum()

        # Weighted covariance
        weighted_centered = centered * np.sqrt(dist_weights)[:, np.newaxis]
        cov = weighted_centered.T @ weighted_centered

        vals, vecs = np.linalg.eigh(cov)
        idx_max = int(np.argmax(vals))
        principal = vecs[:, idx_max]
        
        if float(np.dot(principal, outward_ref)) < 0.0:
            principal = -principal
            
        var_sum = float(np.sum(vals))
        linearity = float(vals[idx_max] / (var_sum + 1e-12))
        directional = max(0.0, float(np.dot(principal, outward_ref)))
        conf = float(np.clip(
            ENDPOINT_PCA_CONFIDENCE_LINEARITY_WEIGHT * linearity
            + ENDPOINT_PCA_CONFIDENCE_DIRECTION_WEIGHT * directional,
            0.0, 1.0
        ))
        
        directions.append(principal)
        weights.append(conf)
        
    mean_dir = np.zeros(2, dtype=np.float64)
    for d, w in zip(directions, weights):
        mean_dir += d * w
        
    norm = np.linalg.norm(mean_dir)
    if norm < 1e-12:
        return outward_ref, 0.0
        
    final_tangent = mean_dir / norm
    R = norm / sum(weights) if sum(weights) > 0 else 0.0
    final_conf = float(np.clip(R * np.mean(weights), 0.0, 1.0))
    
    return final_tangent, final_conf


def _estimate_endpoint_context_tangent(
    path: BezierPath,
    end: str,
) -> Tuple[np.ndarray, float]:
    """Estimate endpoint tangent from local sampled context and return confidence."""
    sampled = path.sample(pts_per_segment=ENDPOINT_SAMPLE_PTS_PER_SEGMENT)
    sampled = _dedupe_polyline(sampled)
    # Improvement #10: re-enable sway stripping with conservative parameters.
    # Only strip on paths with enough data (>20 points) to avoid over-trimming.
    if len(sampled) > 20:
        sampled = _strip_sway_prefix(sampled, end, max_strip_fraction=0.08)
    
    if len(sampled) < ENDPOINT_SAMPLE_MIN_POINTS:
        return np.array([1.0, 0.0], dtype=np.float64), 0.0

    if end == "start":
        path_dir = sampled[min(ENDPOINT_LOOKAHEAD_DISTANCE, len(sampled)-1)] - sampled[0]
        outward_ref = _safe_normalize(-path_dir)
    else:
        path_dir = sampled[-1] - sampled[max(0, len(sampled)-ENDPOINT_LOOKBACK_DISTANCE)]
        outward_ref = _safe_normalize(path_dir)

    return _multiscale_context_tangent(sampled, end, outward_ref)


# ═══════════════════════════════════════════════════════════════════════════
# Endpoint extraction
# ═══════════════════════════════════════════════════════════════════════════

def _compute_endpoint_tangent(path: BezierPath, end: str) -> Tuple[np.ndarray, float]:
    """Outward endpoint tangent blended with local context and confidence."""
    if end == "start":
        cp = path.segments[0].control_points
        derivative_tangent = _safe_normalize(-_bezier_derivative_at(cp, 0.0))
    else:
        cp = path.segments[-1].control_points
        derivative_tangent = _safe_normalize(_bezier_derivative_at(cp, 1.0))

    context_tangent, confidence = _estimate_endpoint_context_tangent(path, end)
    
    if confidence < ENDPOINT_CONFIDENCE_LOW_THRESHOLD:
        blend = float(np.clip(confidence, ENDPOINT_BLEND_LOW_CONFIDENCE_MIN, ENDPOINT_BLEND_LOW_CONFIDENCE_MAX))
    else:
        blend = float(np.clip(confidence, ENDPOINT_BLEND_HIGH_CONFIDENCE_MIN, ENDPOINT_BLEND_HIGH_CONFIDENCE_MAX))
        
    tangent = _safe_normalize((1.0 - blend) * derivative_tangent + blend * context_tangent)
    
    if end == "start":
        interior_dir = _safe_normalize(path.segments[0].control_points[3] - path.segments[0].control_points[0])
    else:
        interior_dir = _safe_normalize(path.segments[-1].control_points[0] - path.segments[-1].control_points[3])
        
    if float(np.dot(tangent, interior_dir)) > ENDPOINT_INTERIOR_ALIGNMENT_THRESHOLD:
        tangent = -tangent
        confidence *= ENDPOINT_MISALIGNMENT_CONFIDENCE_PENALTY
        
    return tangent, confidence


def _compute_endpoint_curvature(path: BezierPath, end: str) -> float:
    """Unsigned curvature at the given end of the path."""
    if end == "start":
        cp = path.segments[0].control_points
        return _curvature_at(cp, 0.0)
    else:
        cp = path.segments[-1].control_points
        return _curvature_at(cp, 1.0)


def _extract_endpoints(paths: List[BezierPath]) -> List[EndpointInfo]:
    """Build EndpointInfo objects for every open path's start and end."""
    endpoints: List[EndpointInfo] = []
    next_endpoint_id = 0
    for idx, path in enumerate(paths):
        if path.is_closed or not path.segments:
            continue
        for end in ("start", "end"):
            pos = (
                path.segments[0].control_points[0]
                if end == "start"
                else path.segments[-1].control_points[3]
            )
            tangent, tangent_confidence = _compute_endpoint_tangent(path, end)
            endpoints.append(EndpointInfo(
                endpoint_id=next_endpoint_id,
                path_index=idx,
                end=end,
                position=pos.astype(np.float64),
                tangent=tangent,
                curvature=_compute_endpoint_curvature(path, end),
                tangent_confidence=tangent_confidence,
            ))
            next_endpoint_id += 1
    return endpoints


# ═══════════════════════════════════════════════════════════════════════════
# EFD extraction (wraps existing module)
# ═══════════════════════════════════════════════════════════════════════════

def _extract_efd_contours(
    image_path: str,
    min_contour_area: float = 100.0,
    order: int = 40,
) -> List[dict]:
    """Extract EFD coefficients for closed contours in the image."""
    import pyefd

    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        return []

    _, binary = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)

    contours, _ = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    results: List[dict] = []
    for cnt in contours:
        if cv2.contourArea(cnt) < min_contour_area:
            continue
        hull = cv2.convexHull(cnt)
        hull_area = cv2.contourArea(hull)
        if hull_area < 1e-6:
            continue
        solidity = cv2.contourArea(cnt) / hull_area
        if solidity < 0.45:
            continue

        _, (w_rect, h_rect), _ = cv2.minAreaRect(cnt)
        longer = max(w_rect, h_rect)
        shorter = min(w_rect, h_rect)
        if shorter < 1e-6 or (longer / shorter) > 8.0:
            continue

        coords = np.squeeze(cnt)
        if coords.ndim != 2 or len(coords) < 5:
            continue
        try:
            coeffs = pyefd.elliptic_fourier_descriptors(
                coords, order=order, normalize=False,
            )
            a0, c0 = pyefd.calculate_dc_coefficients(coords)
            results.append({
                "contour": coords,
                "coeffs": coeffs,
                "locus": (a0, c0),
                "order": order,
            })
        except (ValueError, cv2.error):
            continue
    return results


# ═══════════════════════════════════════════════════════════════════════════
# Main entry point
# ═══════════════════════════════════════════════════════════════════════════

def extract_paths(
    image_path: str,
    max_error: float = 2.0,
    spur_threshold: float = 12.0,
    merge_radius: float = 5.0,
    tangent_lookahead: int = 8,
    straightness_scale: float = 0.75,
    junction_min_alignment: float = -0.15,
) -> ExtractionResult:
    """Extract Bezier paths, endpoints, and EFD data from a raster image.

    Improvement #13: exposes previously hidden parameters from
    fit_from_image_skeleton so the pipeline can be tuned without
    modifying source code.

    Args:
        image_path: path to the raster sketch image.
        max_error: maximum Schneider fitting error (pixels) before splitting.
        spur_threshold: arc-length threshold for skeleton spur pruning.
        merge_radius: maximum pixel distance to merge connected path endpoints.
        tangent_lookahead: number of pixels to look ahead for tangent estimation.
        straightness_scale: higher values favour straight cubic approximations.
        junction_min_alignment: minimum dot-product alignment for junction
            continuation (-1 to 1, lower values are more permissive).
    """
    img = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    if img is None:
        raise FileNotFoundError(f"Cannot read image: {image_path}")
    h, w = img.shape[:2]
    diagonal = float(np.hypot(h, w))

    # Bezier path extraction via skeleton
    paths, _ = fit_from_image_skeleton(
        image_path,
        max_error=max_error,
        spur_threshold=spur_threshold,
        merge_radius=merge_radius,
        tangent_lookahead=tangent_lookahead,
        straightness_scale=straightness_scale,
        junction_min_alignment=junction_min_alignment,
    )

    # Ensure visual discovery order (top-down, left-right)
    def path_sort_key(p: BezierPath):
        pts = p.sample(5)
        min_y = np.min(pts[:, 1])
        min_x = np.min(pts[:, 0])
        return (round(min_y / 12.0), min_x)
    
    paths.sort(key=path_sort_key)

    # Endpoint extraction
    endpoints = _extract_endpoints(paths)

    # EFD extraction
    efd_contours = _extract_efd_contours(image_path)

    # Compute distance map from the original image (lines=0, background=255)
    _, binary = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    dist_map = cv2.distanceTransform((255 - binary).astype(np.uint8), cv2.DIST_L2, 5)

    return ExtractionResult(
        paths=paths,
        endpoints=endpoints,
        efd_contours=efd_contours,
        image_shape=(h, w),
        diagonal=diagonal,
        dist_map=dist_map,
    )
